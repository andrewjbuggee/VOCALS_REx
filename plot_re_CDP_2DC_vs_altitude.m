...
function [] = plot_re_CDP_2DC_vs_altitude(vert_profiles, indices, normalize_altitude)
...
for nn = 1:N_plots
    ...
    if normalize_altitude==true
        ...
        plot(vert_profiles.re_CDP{indices(nn)}, norm_alt, 'Color', mySavedColors(1, 'fixed'));
        hold on
        ...
        % available
        if isfield(vert_profiles, 're_2DC')==true
            plot(vert_profiles.re_2DC{indices(nn)}, norm_alt, 'Color', mySavedColors(2, 'fixed'));

        elseif isfield(vert_profiles, 'mean_r_2DC')==true
            plot(vert_profiles.mean_r_2DC{indices(nn)}, norm_alt, 'Color', mySavedColors(2, 'fixed'));
        ...
        % Plot the CDP re first
        plot(vert_profiles.re_CDP{indices(nn)}, vert_profiles.altitude{indices(nn)},...
