%% Plot the medain profile for LWC, re, and Nc versus normalized horizontal distance

% only use the profiles defined by the logical index provided


function plot_median_horizontal_ensemble_LWC_re_NC_vs_norm_distance(ensemble_profiles, index)
...
% we break up the horizontal component into n discrete bins. Within each bin we
% can compute the mean, median and standard deviation

n_bins = 45; % number of segments the noramlized horizontal component is broken up into
...
normalized_distance = cell(1, length(index));


for nn = 1:length(index)

    % first we need to normalize the vertical component of all profiles
    normalized_distance{nn} = (ensemble_profiles.horz_dist{nn} - min(ensemble_profiles.horz_dist{nn}))./...
                               (max(ensemble_profiles.horz_dist{nn}) - min(ensemble_profiles.horz_dist{nn}));
    ...
    for bb = 1:length(bin_edges)-1
        ...
        % accordingly
        if bb==1
            index_segment = normalized_distance{nn}>=bin_edges(bb) & normalized_distance{nn}<=bin_edges(bb+1);
        ...
        horizontally_segmented_attributes{bb, 2} = [horizontally_segmented_attributes{bb, 2}; lwc(index_segment)];

        % store the total droplet number concentration values
        horizontally_segmented_attributes{bb, 3} = [horizontally_segmented_attributes{bb, 3}; Nc(index_segment)];
...
lwc_reject_normal = zeros(1, size(horizontally_segmented_attributes,1));
lwc_p_normal = zeros(1, size(horizontally_segmented_attributes,1));

lwc_reject_lognormal = zeros(1, size(horizontally_segmented_attributes,1));
lwc_p_lognormal = zeros(1, size(horizontally_segmented_attributes,1));

lwc_reject_gamma = zeros(1, size(horizontally_segmented_attributes,1));
lwc_p_gamma = zeros(1, size(horizontally_segmented_attributes,1));



Nc_reject_normal = zeros(1, size(horizontally_segmented_attributes,1));
Nc_p_normal = zeros(1, size(horizontally_segmented_attributes,1));

Nc_reject_lognormal = zeros(1, size(horizontally_segmented_attributes,1));
Nc_p_lognormal = zeros(1, size(horizontally_segmented_attributes,1));

Nc_reject_gamma = zeros(1, size(horizontally_segmented_attributes,1));
Nc_p_gamma = zeros(1, size(horizontally_segmented_attributes,1));
...
%     set(gcf, 'Position', [0 0 1200 500])
    ...
    lwc_fit_normal(bb) = fitdist(horizontally_segmented_attributes{bb,2}, 'normal');
    [lwc_reject_normal(bb), lwc_p_normal(bb)] = chi2gof(horizontally_segmented_attributes{bb,2}, 'CDF',lwc_fit_normal(bb));

    % fit the liquid water content data to a log-normal distribution
    lwc_fit_lognormal(bb) = fitdist(horizontally_segmented_attributes{bb,2}, 'lognormal');
    [lwc_reject_lognormal(bb), lwc_p_lognormal(bb)] = chi2gof(horizontally_segmented_attributes{bb,2}, 'CDF',lwc_fit_lognormal(bb));

    % fit the liquid water content data to a gamma distribution
    lwc_fit_gamma(bb) = fitdist(horizontally_segmented_attributes{bb,2}, 'gamma');
    [lwc_reject_gamma(bb), lwc_p_gamma(bb)] = chi2gof(horizontally_segmented_attributes{bb,2}, 'CDF', lwc_fit_gamma(bb));
...
%     set(gcf, 'Position', [0 0 1200 500])
    ...
    Nc_fit_normal(bb) = fitdist(horizontally_segmented_attributes{bb,3}, 'normal');
    [Nc_reject_normal(bb), Nc_p_normal(bb)] = chi2gof(horizontally_segmented_attributes{bb,3}, 'CDF',Nc_fit_normal(bb));

    % fit the number concentration content data to a log-normal distribution
    Nc_fit_lognormal(bb) = fitdist(horizontally_segmented_attributes{bb,3}, 'lognormal');
    [Nc_reject_lognormal(bb), Nc_p_lognormal(bb)] = chi2gof(horizontally_segmented_attributes{bb,3}, 'CDF',Nc_fit_lognormal(bb));

    % fit the number concentration content data to a gamma distribution
    Nc_fit_gamma(bb) = fitdist(horizontally_segmented_attributes{bb,3}, 'gamma');
    [Nc_reject_gamma(bb), Nc_p_gamma(bb)] = chi2gof(horizontally_segmented_attributes{bb,3}, 'CDF', Nc_fit_gamma(bb));
...
% ---- most common best fit distribution for r_e was is the normal dist ---
...
for bb = 1:n_bins
    ...
    re_median(bb) = median(horizontally_segmented_attributes{bb,1});       % microns - mean droplet radius value

    % compute the standard deviation of the current bin
    re_std(bb) = std(horizontally_segmented_attributes{bb,1});         % microns - standard deviation

    % The std is the average deviation from the mean value. Compute the
    % average deviation from the median value
    re_avg_deviation_from_median(bb) = sqrt(mean((horizontally_segmented_attributes{bb,1} - re_median(bb)).^2));
...
%     re_logNormal_std(bb) = sqrt(exp(2*re_fit_lognormal(bb).mu + re_fit_lognormal(bb).sigma^2)*(exp(re_fit_lognormal(bb).sigma^2) - 1));
    ...
    % compute the median value for the current bin
    lwc_median(bb) = median(horizontally_segmented_attributes{bb,2});       % microns - mean droplet radius value

    % compute the standard deviation of the current bin
    lwc_std(bb) = std(horizontally_segmented_attributes{bb,2});         % microns - standard deviation
    ...
    % compute the median value for the current bin
    Nc_median(bb) = median(horizontally_segmented_attributes{bb,3});       % microns - mean droplet radius value

    % compute the standard deviation of the current bin
    Nc_std(bb) = std(horizontally_segmented_attributes{bb,3});         % microns - standard deviation
...
y = [re_median - re_avg_deviation_from_median; flipud(re_median + re_avg_deviation_from_median)];
...
plot(bin_center, re_median, 'Color', mySavedColors(2, 'fixed'))
...
% plot the standard deviation of the median profile as an transparent area
% centered around the mean radius profile
y = [lwc_median-lwc_std; flipud(lwc_median + lwc_std)];
x = [bin_center; flipud(bin_center)];
fill(x,y,mySavedColors(2,'fixed'), 'EdgeAlpha', 0, 'FaceAlpha', 0.2)
...
% plot the median droplet profile
plot(bin_center, lwc_median, 'Color', mySavedColors(2, 'fixed'))


grid on; grid minor
ylabel('$<LWC(z)>$ $(g/m^{3})$', 'Interpreter','latex')
xlabel('Normalized Horizontal Distance', 'Interpreter','latex')

% set the figure title
title(['$LWC \geq$ ', num2str(ensemble_profiles.inputs.LWC_threshold),' $g/m^{3}$',...
        '   $N_c \geq$ ', num2str(ensemble_profiles.inputs.Nc_threshold), ' $cm^{-3}$',...
        '   Max vert displacement: ', num2str(ensemble_profiles.inputs.max_vert_displacement), ' $m$'], 'interpreter', 'latex')
...
% plot the standard deviation of the median profile as an transparent area
% centered around the mean radius profile
y = [Nc_median-Nc_std; flipud(Nc_median + Nc_std)];
x = [bin_center; flipud(bin_center)];
fill(x,y,mySavedColors(2,'fixed'), 'EdgeAlpha', 0, 'FaceAlpha', 0.2)
...
% plot the median droplet profile
plot(bin_center, Nc_median, 'Color', mySavedColors(2, 'fixed'))


grid on; grid minor
ylabel('$<N_c(z)>$ $(cm^{-3})$', 'Interpreter','latex')
